...
// budget-funded session goes back to the owner's wallet from that owner's payer key.
...
export function defaultApproveDeps(): ApproveDeps {
  return {
    chain: {
      ...
      returnToOwner: (owner, amount, payer) => returnToOwner(owner, amount, chainForOwner(payer)),

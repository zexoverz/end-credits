...
import { loginUrl, parseAddress, type ClaimView, type Notice, type StepState } from "@/lib/client/claim";
...
const STATE_LABEL: Record<StepState, ClaimCopyCode> = { done: "DONE", active: "ACTIVE", locked: "LOCKED", failed: "FAILED" };
...
export function Step({ n, title, state, children }: { n: number; title: ClaimCopyCode; state: StepState; children?: ReactNode }) {
  return (
    <li className={`rounded-lg border border-line bg-card p-4 ${state === "locked" ? "opacity-50" : ""}`}>
      <div className="flex items-center justify-between">
        <h2 className="font-semibold">
          {n}. {claimCopy(title)}
        </h2>
        <span className={`text-xs uppercase ${STATE_CLS[state]}`}>{claimCopy(STATE_LABEL[state])}</span>
      </div>
      {state !== "locked" && children && <div className="mt-3 space-y-3 text-sm">{children}</div>}
    </li>
  );
}

const WalletLink = ({ address }: { address: string }) => (
  <a className="underline" href={addressUrl(address)} target="_blank" rel="noreferrer">
    <Mono>{address}</Mono>
  </a>
);

export function GithubStep({ name, repo, login, cancelled }: { name: string; repo: string; login: string | null; cancelled: boolean }) {
  if (login) return <p>{claimCopy("GITHUB_AS", { login })}</p>;
  return (
    <>
      {cancelled && <NoticeLine notice={{ tone: "error", text: claimCopy("GITHUB_CANCELLED"), code: "cancelled" }} />}
      <p className="text-muted">{claimCopy("GITHUB_WHY", { repo })}</p>
      <a href={loginUrl(name)} className="inline-block rounded bg-foreground px-3 py-1.5 text-background">
...
export function WalletStep({ claim, done, busy, notice, onPasskey, onAddress }: WalletProps) {
  ...
  if (done && claim?.wallet) {
    return (
      <p>
        {claimCopy("WALLET_SET")}{" "}
        <WalletLink address={claim.wallet} />
      </p>
    );
  }
  const submit = () => {
  ...
  return (
    <>
      ...
      {invalid && <NoticeLine notice={{ tone: "error", text: claimCopy("WALLET_INVALID"), code: "invalid" }} />}
      <NoticeLine notice={notice} />
    </>
  );
}

type PrProps = { repo: string; claim: ClaimView | null; done: boolean; busy: boolean; notice: Notice | null; onOpen: () => void };
  ...
  return (
    <>
      ...
      {claim.prMode === "new_file_link" ? (
        ...
        <a className="underline" href={claim.prUrl} target="_blank" rel="noreferrer">
...
type MergeProps = { claim: ClaimView | null; state: StepState; busy: boolean; notice: Notice | null; onCheck: () => void };

export function MergeStep({ claim, state, busy, notice, onCheck }: MergeProps) {
  if (state === "done" && claim?.wallet) {
    const text = notice?.code === "CLAIMED" ? notice.text : msg("CLAIMED", { amount: claim.claimedAmount ?? "0", address: claim.wallet });
    return (
      <>
  ...
  return (
    <>
      {state === "active" && <p className="text-muted">{claimCopy("MERGE_WHY")}</p>}
      <NoticeLine notice={notice} />
      {state === "active" && (

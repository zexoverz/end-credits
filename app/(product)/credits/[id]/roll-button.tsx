...
export function RollButton({ id, onRequested }: { id: string; onRequested: () => void }) {
  ...
  async function roll() {
    setBusy(true);
    try {
      const res = await api(`/api/sessions/${encodeURIComponent(id)}/settle`, { method: "POST" });
  ...
  return (
    <div className="my-6 flex flex-col items-center gap-2 text-sm">
      ...
      {result && (
        <p className={result.kind === "requested" ? "text-white/70" : "text-held"}>
          {result.text}
          {result.kind === "sign_in" && (
            <Link href="/owner" className="ml-2 underline">

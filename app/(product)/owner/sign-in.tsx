...
export function SignIn({ worldCode, onSignedIn }: { worldCode: string | null; onSignedIn: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    setError(null);
    const r = await api("/api/auth/dev", { method: "POST", body: JSON.stringify({ token }) });
    setBusy(false);
    if (!r.ok) {
  ...
  return (
    <div className="flex max-w-md flex-col gap-4">
      ...
      <Card>
        ...
        <a href="/api/auth/world/start" className="inline-block rounded bg-foreground px-3 py-2 text-sm text-background">
          {C.WORLD_SIGN_IN}
        </a>
      </Card>
      <Card>

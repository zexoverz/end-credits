...
import { settleNext, type DepsFor } from "../lib/settle/run";
import type { SettleDeps } from "../lib/settle/settle";
import { startLoop, type Loop } from "./loop";

export const SETTLE_INTERVAL_MS = 2_000;

export function startSettler(deps: SettleDeps, log: (line: string) => void = console.error, depsFor?: DepsFor): Loop {
  return startLoop("settler", SETTLE_INTERVAL_MS, () => settleNext({ log, ...deps }, depsFor), log);
}

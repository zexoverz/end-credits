// Worker process: the provisioner (every 5 s), the settler (every 2 s) and the expirer (every 30 s). SIGTERM or SIGINT lets the
...
import { startProvisioner } from "./provisioner";
...
const loops = [
  startProvisioner(log),
  startSettler(settleDepsFromEnv(process.env, log), log, settleDepsPerOwner(process.env, log)),
  startExpirer(db(), chain(), log),
];
